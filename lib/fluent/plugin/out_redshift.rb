module Fluent
  class RedshiftOutput < BufferedOutput
    ...
    config_param :redshift_arn, :string, :default => nil
    # file format
    ...
    def configure(conf)
      ...
      @copy_sql_template = "copy #{table_name_with_schema} from '%s' CREDENTIALS 'aws_iam_role=#{redshift_arn}' delimiter '#{@delimiter}' GZIP ESCAPE #{@redshift_copy_base_options} #{@redshift_copy_options};"
    end
